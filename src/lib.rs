  ...
  for &field in FIELDS_ORDER {
    if let Some(value) = obj.get(field) {
      let transformed = match (field, value) {
        ...
        (
          "keywords" | "files" | "bundledDependencies" | "bundleDependencies" | "categories"
          | "activationEvents" | "extensionPack" | "extensionDependencies",
          Value::Array(arr),
        ) => Value::Array(sort_array_unique(arr)),

        // Scripts sorting
        ("scripts" | "betterScripts", Value::Object(obj)) => Value::Object(sort_scripts(obj)),
